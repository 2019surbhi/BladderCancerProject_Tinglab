...
path<-'/home/sonas/tingalab/Surbhi/PROJECTS_tinglab_drive/BLADDER/copyKat/genes/gain_loss_subset/samples17_per_clade/thresholds/'

out<-'/home/sonas/copykat/genes/gain_loss/samples17_per_clade/threshold_plots/'
...
#nm<-parse_number(gsub('samples17','',f))

file1<-f[grep('clade5',f)]
nm<-parse_number(gsub('samples17','',file1) %>% gsub('clade[0-9]','',.))
...
p90_coord<-lapply(p90,nrow) %>% unlist()
p80_coord<-lapply(p80,nrow) %>% unlist()
p75_coord<-lapply(p75,nrow) %>% unlist()
p70_coord<-lapply(p70,nrow) %>% unlist()
...
df90<-data.frame("genomic_segments"=p90_coord,"percentile"=rep('90p',length(p90_coord)))
df80<-data.frame("genomic_segments"=p80_coord,"percentile"=rep('80p',length(p80_coord)))
df75<-data.frame("genomic_segments"=p75_coord,"percentile"=rep('75p',length(p75_coord)))
df70<-data.frame("genomic_segments"=p70_coord,"percentile"=rep('70p',length(p70_coord)))
